------------------------------------------------------------
-- PNB Controller (Versi HUB)
-- Integrasi penuh dengan sistem HUB + GUI
------------------------------------------------------------
-- ## SC CREATED BY Void_String ## --
-- ## My Discord: discord.com/invite/psdQaVEnHt ## --
-- ## Customize HUB By Server048 
...
local worldvend = "off"
 -- arroz position
local aVposX, aVposY = 186, 196
 -- clover position
local cVposX, cVposY = 186, 196
---------------------------------
...
-- 2) STATE
-----------------------------
local HOOK_NAME  = "PNB_Controller"
local pnbThread  = nil
local statusText = "Idle"

------------------------------------------------------------
-- 3) StopCheck fallback
------------------------------------------------------------
...
AddHook("OnDraw", HOOK_NAME, function()
  ...
  if ImGui.Begin("PNB Controller", true, ImGuiWindowFlags_AlwaysAutoResize) then
    ...
    -- Panel konfigurasi (lanjutkan dari script aslimu)
    ImGui.Separator()
...
-- 7) PNB CORE
------------------------------------------------------------
-- Semua fungsi asli (MagplantManager, fnd, sycroneC, pnbmain, dsb)
-- langsung dari script aslimu ditempel di bawah sini
------------------------------------------------------------

-- [ ⬇ letakkan full kode PNB kamu (MagplantManager, hooks, pnbmain, dll) ⬇ ]
-- Pastikan pnbmain() tetap dipanggil di loop wrapper atas
