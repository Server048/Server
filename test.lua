  ...
  if str == "a" then
    ...
    for y = Customize.Start.PosY + 1, Customize.Start.PosY % 2 == 0 and 0 or 1, -2 do
      for x = Customize.Start.PosX, 199, 1 do
        ...
        if ShouldStop() then return Total end
      end
    end
    return Total
  else -- "b"
    for y = Customize.Start.PosY, Customize.Start.PosY % 2 == 0 and 0 or 1, -2 do
      for x = Customize.Start.PosX, 199, 1 do
        ...
        if ShouldStop() then return TotalTree end
      end
    end
    return TotalTree
  end
...
AddHook("OnDraw", "PTHT_GUI", function()
  -- jika dimatikan oleh HUB, sembunyikan GUI & stop semua
  ...
  if ImGui.Button("❌ Tutup GUI", 288, 32) then
    -- terminate total: patuh HUB
    pt_running = false
    ui_visible = false
    _G.shouldStop = true  -- sinyal ke HUB untuk hentikan eksekusi
    TextO("`4[PTHT] Terminate requested.")
  end

  ImGui.Separator()
  ImGui.Text("World     : "..World)
  ImGui.Text("Mode      : "..(Plant and (Harvest and "Mix?" or "Plant") or (Harvest and "Harvest" or "Unknown")))
